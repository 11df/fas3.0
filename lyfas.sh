...
function web() {
#安装web云端
rm -rf /var/www/* && cd /var/www && wget -q https://raw.githubusercontent.com/h1d3r/FAS3.0/master/fas_web.zip #http://lingyi3.nos-eastchina1.126.net/fas_web.zip #${http}${host}${ly863963860}fas_web.zip
...
#导入数据库vpndata表数据
sed -i "s/JuLi_User/"$lyadminuser"/g" /var/www/vpndata.sql
sed -i "s/JuLi_Pass/"$lyadminpass"/g" /var/www/vpndata.sql
sed -i "s/服务器IP/"$IP"/g" /var/www/vpndata.sql
mysql -uroot -p$lysqlpass vpndata < /var/www/vpndata.sql
rm -rf /var/www/vpndata.sql
#修改后台名
mv /var/www/html/phpMyAdmin /var/www/html/$lysqldizhi
...
Crackauthor="H1d3r";
http=https://
host=github.com/H1d3r/FAS3.0/raw/
ly863963860=master/
logo
}
main
